...
import { MapPin, Anchor, Sun, Camera, Instagram, MessageCircle, ChevronRight, ChevronLeft, Star, Menu, X, Bot, SendHorizontal, Clock } from 'lucide-react';
...
export default function PublicSite() {
  ...
  const fetchApprovedComments = async () => {
    try {
      const res = await fetch('/api/comments?status=approved');
      const data = await res.json();
      setComments(data);
    } catch (error) {
    ...
    try {
      const res = await fetch('/api/comments', {
        ...
        body: JSON.stringify({ 
          ...
          content: `${commentForm.rating} estrelas: ${commentForm.text}` 
        }),
      });
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-serif text-sand-900 mb-6">
              Últimas <span className="italic">Atualizações</span>
            </h2>
          </div>

          {updates.length === 0 ? (
            <div className="bg-white p-8 rounded-xl shadow-sm border border-sand-200 text-center text-sand-600">
              Nenhuma atualização publicada ainda.
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {updates.map((update) => (
                <article key={update.id} className="bg-white p-8 rounded-2xl shadow-sm border border-sand-200 transition-shadow hover:shadow-md">
                  <h3 className="text-2xl font-bold text-sand-900 mb-3 font-serif">{update.title}</h3>
                  <div className="flex items-center gap-2 text-sm text-ocean-700 mb-5 font-mono bg-ocean-50 inline-flex px-3 py-1 rounded-full">
                    <Clock className="w-4 h-4" />
                    {new Date(update.created_at).toLocaleDateString('pt-BR', {
                      day: '2-digit', month: 'long', year: 'numeric'
                    })}
                  </div>
                  <p className="text-sand-700 leading-relaxed whitespace-pre-wrap">{update.content}</p>
                </article>
              ))}
            </div>
          )}
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm p-3 rounded-full text-ocean-600 shadow-sm">
                      {tour.icon === 'Sun' && <Sun className="w-6 h-6" />}
                      {tour.icon === 'Anchor' && <Anchor className="w-6 h-6" />}
                      {tour.icon === 'Camera' && <Camera className="w-6 h-6" />}
                      {tour.icon === 'Star' && <Star className="w-6 h-6" />}
                    </div>
                  </div>
      ...
      <section id="depoimentos" className="py-24 px-6 md:px-12 bg-sand-100">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-20">
            {comments.length === 0 ? (
              <p className="text-sand-600 italic text-center col-span-3">Seja o primeiro a avaliar!</p>
            ) : (
              comments.map((comment, idx) => {
                const ratingMatch = comment.content.match(/^(\d) estrelas: (.*)/);
                const rating = ratingMatch ? parseInt(ratingMatch[1]) : 5;
                const text = ratingMatch ? ratingMatch[2] : comment.content;
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-sand-800 font-light italic mb-6 leading-relaxed">"{text}"</p>
                    <p className="font-medium text-sand-900 uppercase tracking-wide text-sm">{comment.author}</p>
